...
public class ResultScreen2 extends Pane {
	...
	public Canvas drawButton(String name, double width, double height, double posX ,double posY) {
		...
		if(name=="Well Done!") {
			...
			gc.fillText("Atk : "+GameManager.getScoreBefore().get(1), 50, height/2-10);
			...
			gc.fillText(GameManager.getScoreBefore().get(2).intValue()+"/"+GameManager.getScoreBefore().get(3).intValue(), 125 , height/2+50);
			...
			gc.fillText("Atk : "+GameManager.getCurrentCha().getAtk(), 50, height/2-10);
			...
			gc.setTextBaseline(VPos.CENTER);
			gc.setFont(SCORE_FONT);
			gc.fillText(GameManager.getCurrentCha().getCurrentExp()+"/"+GameManager.getCurrentCha().getCurrentMaxExp(), 125 , height/2+50);
